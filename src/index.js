...
const ComplaintHandler = require('./commands/complaintHandler');
...
bot.on('message', async (ctx, next) => {
  await GroupAdminHandler.handleGroupServiceMembershipUpdate(ctx);
  return next();
});
...
bot.action(/^tops:(rulers|arena|invaders|thieves|money|divorced_men|divorced_women|married|active|groups|games|hide)$/i, async (ctx) => {
  ...
  if (action === 'divorced_men') {
    return BankGameHandler.handleTopDivorcedMen(ctx);
  }
...
bot.hears(/^(?:\/)?متفاعلين$/i, (ctx) => BankGameHandler.handleTopActiveInGroup(ctx));
...
bot.hears(/^توب\s*المخلوعين$/i, (ctx) => BankGameHandler.handleTopDivorcedMen(ctx));
...
bot.hears(/^(?:\/)?حاكم\s*جلاد(?:\s*متهم)?$/i, (ctx) => GroupGamesHandler.handleRulerExecutionCommand(ctx));
bot.hears(/^(?:\/)?انضم\s*حاكم\s*جلاد(?:\s*متهم)?$/i, (ctx) => GroupGamesHandler.handleRulerExecutionJoinCommand(ctx));
bot.hears(/^(?:\/)?ابد[اأ]\s*حاكم\s*جلاد(?:\s*متهم)?$/i, (ctx) => GroupGamesHandler.handleRulerExecutionBeginCommand(ctx));
bot.hears(/^(?:\/)?(?:انهاء|إنهاء)\s*حاكم\s*جلاد(?:\s*متهم)?$/i, (ctx) => GroupGamesHandler.handleRulerExecutionEndCommand(ctx));
...
bot.hears(/^(?:حسابات|حسابي|حساباتي)(?:\s+.+)?$/i, (ctx) => GroupGamesHandler.handleCalculationsMcqCommand(ctx));
